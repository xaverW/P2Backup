...
public class CellBackuErrorOpenFileButton<S, T> extends TableCell<S, T> {

    private final ObjectProperty<Stage> stageProp;

    public CellBackuErrorOpenFileButton(ObjectProperty<Stage> stageProp) {
            ...
            public void updateItem(String item, boolean empty) {
                ...
                btnOpenDirectory.setOnAction((ActionEvent event) -> {
                    ...
                    if (path != null && path.toFile().exists() && path.toFile().isDirectory()) {
                        P2Open.openDir(stageProp.get(), path.toFile().toString());
                    }

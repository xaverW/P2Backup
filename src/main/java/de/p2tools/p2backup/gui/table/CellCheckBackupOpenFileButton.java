...
public class CellCheckBackupOpenFileButton<S, T> extends TableCell<S, T> {

    private final ObjectProperty<Stage> stageProp;

    public CellCheckBackupOpenFileButton(ObjectProperty<Stage> stageProp) {
    ...
    public final Callback<TableColumn<FileData, String>, TableCell<FileData, String>> cellFactory
        ...
        final TableCell<FileData, String> cell = new TableCell<>() {
            ...
            public void updateItem(String item, boolean empty) {
                ...
                btnStart.setDisable(!fileData.isExistInBackup() || fileData.isError());

                btnStart.setOnAction((ActionEvent event) -> {
                    getTableView().getSelectionModel().clearSelection();
                    getTableView().getSelectionModel().select(getIndex());
                    P2Open.openFile(stageProp.get(), fileData.getBackupFilePathStr());
                ...
                btnOpenDirectory.setOnAction((ActionEvent event) -> {
                    ...
                    if (path != null && path.toFile().exists() && path.toFile().isDirectory()) {
                        P2Open.openDir(stageProp.get(), path.toString());
                    }
